...
def main():
    ...
    with open(os.getcwd() + '/data') as file_obj:
        ...
        for user_email, manager_details in json_data.items():
            ...
            for manager_index, detail in enumerate(manager_details):
                if detail.get('email') in json_data.keys():
                    if manager_data.get(detail.get('email')):
                        if manager_data.get(detail.get('email')) < manager_index + 1:
                            manager_data.update({detail.get('email').lower() : manager_index + 1})
                    else:
                        manager_data.update({detail.get('email') : manager_index + 1})
...
def generate_csv():
    ...
    for data in half_formated_data:
        if data[0] in manager_data.keys(): # Here data[0] is the user_email from the main() function
            ...
            data.append(manager_data[data[0]])
        else:
